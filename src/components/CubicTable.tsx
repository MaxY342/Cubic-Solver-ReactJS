        ...
        <table className="w-full font-mono text-sm border-collapse">
          <tbody>
            <tr className="border-b border-zinc-800">
              <th className="text-left py-1.5 pr-4 text-zinc-500 font-normal w-1/3">
                p
              </th>
              <td
                className="py-1.5 text-zinc-300 text-right tabular-nums"
                colSpan={2}
              >
            ...
            <tr className="border-b border-zinc-700">
              ...
              <th className="py-2 text-red-500/70 text-xs tracking-widest uppercase font-bold text-center">
                x
              </th>
              <th className="py-2 text-red-500/70 text-xs tracking-widest uppercase font-bold text-center">
                y
            ...
            {computed?.roots.map((root: any, idx: number) => (
              <tr key={idx} className="border-b border-zinc-800/60">
                <th className="text-left py-1.5 pr-4 text-zinc-500 font-normal">
                  Root {idx + 1}
                </th>
                <td
                  className={`py-1.5 text-center tabular-nums ${root.y === "Imaginary" ? "text-zinc-700 italic" : "text-red-400"}`}
                >
                  {root.x}
                </td>
                <td
                  className={`py-1.5 text-center tabular-nums ${root.y === "Imaginary" ? "text-zinc-700 italic" : "text-zinc-400"}`}
                >
                  {root.y}
                </td>
              </tr>
